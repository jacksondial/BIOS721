...
ggplot(df3, aes(x = gdpPercap, y = lifeExp, group = country))+
  geom_point(aes(col = country))+
  geom_line(aes(col = country))+
  facet_wrap(~country, scales = "free")

ggplot(df3, aes(x = gdpPercap, y = lifeExp))+
  geom_point(aes(alpha = year, col = country, size = 4), shape = 16)


ggplot(df3, aes(x = gdpPercap, y = lifeExp))+
  geom_point(aes(col = as.factor(year)))+
  facet_wrap(~country, scales = "free")+
  scale_fill_gradientn(colors = wes_palette("Zissou1", 12, type = "continuous"))+
  labs(x = "GDP Per Capita ($)",
       y = "Life Expectancy (Years)",
